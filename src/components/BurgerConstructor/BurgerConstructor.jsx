...
function BurgerConstructor() {
  ...
  const { burgerIngredients, order, ingredients, bun, totalSum, flag } = useSelector(state => state.ingr)
    ...
    <section className={`${sty.burgerConstructor} mt-15 ml-5 `}>
      <div className={`${sty.constructor}`} ref={drop}>
        <div className={sty.topBulka}>
          {flag
            ...
            : <div className={`${sty.topBulka__wrap}`}>
              Булка
            </div>
          }
        </div>
        <ul className={`${sty.wrap} custom-scroll`}>
          {!flag
            ...
            : burgerIngredients.map((item, index) => {
              return <ConstructorItem key={uuid()}  item={item} index={index} moveCard={moveIngredient}/>
        ...
        <div className={sty.bottomBulka}>
          {!flag
            ?
            <div className={`${sty.bottomBulka__wrap}`}>
              Булка
            </div>
            :
            bun.map((item) => {
      ...
      <div className={`${sty.footer} pt-5`} >
        <div className={`${sty.price} mr-10`}>
          <p className={`${sty.counter} text text_type_main-large mr-2`}>{!flag ? 0 : totalSum}</p>
