...
function BurgerIngredients() {
	...
	useEffect(() => {
		...
		const handleScroll = () => {
			...
			// if (parentTop - bunTop > 0 && !(parentTop - sauceTop > -40)) {
	...
	return (
		<section className={`pt-5 ${sty.constructor} mr-5`}>
			...
			<div className={`mb-5 mt-5`}>
				<button className={`${sty.button} text text_type_main-default text_color_inactive ${activ === 'bun' ? sty.activ : null}`}>
					Булки
				</button>
				<button className={`${sty.button} text text_type_main-default text_color_inactive ${activ === 'sauce' ? sty.activ : null}`}>
					Соусы
				</button>
				<button className={`${sty.button} text text_type_main-default text_color_inactive ${activ === 'main' ? sty.activ : null}`}>
					Начинка
				</button>
			</div>
			{!dataSuccess ? <p>'Загрузка'</p> : (
				<div className={`${sty.cards} custom-scroll`} ref={parentRef}>
					...
					<ul className={`${sty.menu} pl-4 pr-4`}>
						{data.map(item => <MenuItem key={uuid()} item={item} type={'bun'} />)}
					</ul>
					<h3 className={`${sty.teg} text text_type_main-medium mb-3`} ref={sauceRef}>
						Соусы
					</h3>
					<ul className={`${sty.menu} pl-4 pr-4`}>
						{data.map(item => <MenuItem key={uuid()} item={item} type={'sauce'} />)}
					</ul>
					<h3 className={`text text_type_main-medium mb-3`} ref={mainRef}>
						Начинка
					</h3>
					<ul className={`${sty.menu} pl-4 pr-4`}>
						{data.map(item => <MenuItem key={uuid()} item={item} type={'main'} />)}
					</ul>
				</div>
			)
			}
